...
{
    CBarnacoinAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered barnacoin address / amount:
            ...
            scriptPubKey.SetDestination(CBarnacoinAddress(rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal barnacoin:URI (barnacoin:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CBarnacoinAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CBarnacoinAddress(address).ToString().c_str()].push_back(out);
    }
}
...
{
    CTxDestination dest = CBarnacoinAddress(sAddress).Get();
